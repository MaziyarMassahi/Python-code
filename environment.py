...
from constants import SUMMARY_FIELDS
...
class Company():
    ...
    def setup_summary_writer(self):
        ...
        target_dir = 'charts'
        return SummaryWriter(target_dir)
    ...
    def run(self):
        ...
        if self.current_time >= self.simulation_time:
            ...
            try:
                ...
                state = self.vehicles[0].get_state_logs()
            self.logger.debug(f'episod ended - last state returned is : {state}')
            summary = self.get_summary()

            mode = 'a' if os.path.exists('extra/summary.csv') else 'w'
            with open('extra/summary.csv', mode) as f:
                writer = DictWriter(f, fieldnames= SUMMARY_FIELDS )
                if mode == 'w' :
                    writer.writeheader()

                for item in summary:
                    writer.writerow(item)
                self.logger.info(f'summary info written into extra/summary.csv' )
