...
class Part():
    ...
    def reduce_health(self, time, current_time):
        '''time means the time frame that agv was working (on any task or just moving )'''
        change = math.floor( (time/ self.life_time) * 100)
    ...
    def extend_life(self, time):
        self.end_of_life += time
...
class Agv():
    ...
    def __init__(self, id,env_id, mechanical_dist_params, electrical_dist_params, battry_dist_params,
                  location_case:int, writer, periodic_maintenence, log_level, buffer_size = 15) -> None:
        self.id = id
        self.env_id = env_id
    ...
    def task_generator(self, time_frame):
        punish = 0
        unregistered = 0
        passed_min = (math.floor( time_frame / TimeFrame.MINUTE) )
        for _ in range(passed_min):
            r = random.random()
            self.logger.debug(f'generated number --> {r} chance of task --> {TimeFrame.CHANCE_OF_TASK}')
            if r > TimeFrame.CHANCE_OF_TASK:
                self.logger.debug(f'new task added to agv buffer')
                if not self.task_buffer.full():
                    self.task_buffer.put( int(1200000 * random.random()) + 1200000)
                else:
    ...
    def event_manager(self, current_time):
        ...
        self.logger.debug(f'buffer tasks : {self.task_buffer.qsize()}')
        ...
        if self.current_time >= self.progressing_event.end and self.progressing_event.event in Events.ACTIVE_EVENTS:
            ...
            # extending life when agv is in standby
            self.electrical.extend_life(self.time_frame)
            self.mechanincal.extend_life(self.time_frame)
    ...
    def log_action_buffer_data(self, actions, accepted):
        mode = 'a' if os.path.exists(f'extra/agv_{self.id}.csv' ) else 'w'
        fieldnames = ['time', 'part', 'action', 'buffer', 'accepted', 'ttf']
        with open(f'extra/agv_{self.id}.csv', mode= mode) as f:
            writer = DictWriter(f, fieldnames= fieldnames)
            if mode == 'w':
                writer.writeheader()
            for part, action in enumerate( actions ):
                part_name = Part.NAME[part] if part !=3 else 'preventive_maintenence'
                if part == Part.MECHANICAL:
                    ttf = self.mechanincal.end_of_life - self.current_time
                elif part == Part.ELECTRICAL :
                    ttf = self.electrical.end_of_life - self.current_time
                elif part == Part.BATTRY:
                    ttf = self.battry.end_of_life - self.current_time
                else:
                    ttf = self.periodic_maintenence - self.current_time
                if action != 0:
                    writer.writerow({
    ...
    def check_chosen_actions(self, actions:numpy.ndarray):
        '''
        returns a tupel (masked actions, punish)
        '''
        org_act = actions.copy()
        self.logger.debug(f'decided actions : {actions}')
        punish = 0
        preventive = actions[3]
        actions = actions[0:3]
        
        accepted = True if self.task_buffer._qsize() < TimeFrame.BUFFER_LIMIT * self.task_buffer.maxsize else False
        if not self.is_broken():
             self.log_action_buffer_data(org_act, accepted)


        if accepted == False:
            new_actions = numpy.zeros_like(org_act)
            if self.mechanincal.p_health == 0:
                new_actions[0] = 1
            if self.electrical.p_health == 0:
                new_actions[1] = 1
            if self.battry.p_health == 0:
                new_actions[2] = 1
            if self.is_broken():
                self.log_action_buffer_data(new_actions, accepted)
            return new_actions, punish
        ...
        for part, action in enumerate(actions):
            ...
            if part == Part.ELECTRICAL:
                if action == Action.CORRECTIVE_MAINTENENCE and self.electrical.p_health > 20 :
                    ...
                    and (not self.electrical.abnormal or self.electrical.p_health ==0 ):
                    punish += ConstRewards.INVALID_ACTION                    
                    actions[part] = Action.CONTINUE
            if part == Part.BATTRY:
        ...
        if preventive == Action.Preventive.PREVENTIVE_MAINTENENCE:
            if self.current_time < self.periodic_maintenence:
                ...
                self.logger.debug(f'Periodic maintenance executed correctly at {self.current_time}, schedule: {self.periodic_maintenence}')

        elif preventive == Action.Preventive.CONTINUE and self.current_time > self.periodic_maintenence:
            punish += max(0, (self.periodic_maintenence - self.current_time)) * 10**5

        # بررسی اقدامات برای هر قطعه
        for part, action in enumerate(actions):
            component = None
            if part == Part.MECHANICAL:
                component = self.mechanincal
            elif part == Part.ELECTRICAL:
                component = self.electrical
            elif part == Part.BATTRY:
                component = self.battry

            if component:
                if action == Action.CORRECTIVE_MAINTENENCE and component.p_health > 20:
                    punish += ConstRewards.INVALID_ACTION
                    actions[part] = Action.CONTINUE

                elif action != Action.CORRECTIVE_MAINTENENCE and component.p_health == 0:
                    punish += ConstRewards.BIG_PUNISH

                elif action in [Action.PD_REPAIR, Action.PD_REPLACE] and (not component.abnormal or component.p_health == 0):
                    punish += ConstRewards.INVALID_ACTION
                    actions[part] = Action.CONTINUE

                if part == Part.BATTRY:
                    if action == Action.Battry.CHARGE and component.value > 20:
                        punish += ConstRewards.INVALID_ACTION
                        actions[part] = Action.Battry.CONTINUE

            # بازگشت مقادیر
        result = numpy.append(actions, preventive)
